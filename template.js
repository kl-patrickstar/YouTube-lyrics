// ============================================================
//  template.js
//  Static UI definition: CSS, SVG icons, and HTML markup.
//
//  Everything here is a plain string that ui.js injects into
//  the Shadow DOM at runtime. No logic, no event handlers —
//  that all lives in ui.js.
//
//  Why a single file for all three?
//    - Keeps the visual definition together (easier to restyle)
//    - ui.js only needs to reference Y.template
//    - No build step or import system required
//
//  Structure:
//    css   – all panel styles (scoped via :host to the shadow root)
//    ICONS – reusable inline SVG markup, keyed by name
//    html  – the panel markup, ready to be injected
//
//  Design tokens (colors, spacing, motion) are defined as CSS
//  custom properties on :host, so ui.js can tweak them at runtime
//  (e.g. --lyrics-font-size).
//
//  Layout overview:
//    #toggle         – floating mic button (draggable)
//    #panel          – main lyrics panel, open/close animated
//      .panel-header – cover, title, actions, mode switch
//      #editor       – manual search drawer
//      #settings     – settings drawer
//      #status       – info/error banner
//      #lyrics       – scrollable lyrics container
//      #offset-row   – timing pill + external links + actions
//      .source-line  – data source footer
//      #toast        – short-lived confirmation pill
// ============================================================
...
(() => {
   ...
   // ============================================================
   //  CSS
   //  All styles for the panel. Injected into the shadow root,
   //  so nothing here can leak into the YouTube page and vice
   //  versa. Uses CSS custom properties for theming.
   // ============================================================
   const css = `
   :host {
     all: initial;
     ...
     /* --- Popover timing (used by ui.js — keep in sync) --- */
   ...
   * { box-sizing: border-box; }
   ...
   /* ============================================================
      DRAGGING STATE (Chrome speedups)
      During drag we drop backdrop-filter and use a solid
      background — much cheaper for Chrome to composite.
      ============================================================ */
   #panel.dragging {
   ...
   /* ============================================================
      ...
      Panel scales from a small point at the button corner.
      transform-origin is set dynamically by ui.js.
      ============================================================ */
   ...
   /* Segmented control: Sing Along / Full Lyrics */
   .header-segmented {
     margin: 0;
     padding: 2px;
   ...
   /* Small icon-only buttons (header actions) */
   .icon-btn {
     width: 30px;
     height: 30px;
     display: inline-flex;
   ...
   /* ============================================================
      DRAWER (Editor + Settings) — overlay above the panel body
      ============================================================ */
   .drawer {
     position: absolute;
     top: 72px;
   ...
   /* iOS-style switch (used for Auto-scroll) */
   .switch-toggle {
     position: relative;
     width: 36px;
     height: 22px;
   ...
   .status-close svg { width: 12px; height: 12px; }
   ...
   #status.error .status-close { color: var(--danger); }
   ...
   /* Individual lyric lines (karaoke mode) */
   .line {
     margin: 2px 0;
     padding: 9px 12px;
     border-radius: var(--radius-md);
   ...
   /* Fade-in + slide-up when lines first appear */
   ...
   /* Accent ring when the offset is not zero */
   ...
   /* Fill animation when the button is pressed (tap + hold) */
   .pill-btn::before {
     content: "";
     position: absolute;
     inset: 0;
   ...
   /* Offset value display (clickable to reset) */
   #offset-value {
     min-width: 52px;
     height: 26px;
     padding: 0 10px;
   ...
   /* ============================================================
      CUSTOM TOOLTIPS
      Small floating labels above buttons. Shown on hover/focus.
      ============================================================ */
   ...
   /* Right-aligned tooltip variant (for edge buttons) */
   ...
   /* ============================================================
      EXTERNAL LINK BUTTONS (Apple Music, Spotify, UG, Copy, Print)
      ============================================================ */
   .ext-btn {
     display: inline-flex;
     align-items: center;
   ...
   /* ============================================================
      TOAST (copy confirmation, auto-hides)
      ============================================================ */
   .toast {
     position: absolute;
     left: 50%;
   ...
   /* ============================================================
      SKELETON LOADER (while fetching lyrics)
      ============================================================ */
   ...
   /* ============================================================
      ...
      Applied when the panel has the .theme-light class.
      ============================================================ */
   #panel.theme-light {
   ...
   `;
    ...
    // ============================================================
    //  ICONS
    //  Inline SVG strings, one per icon. All icons use
    //  `currentColor` for stroke/fill so they inherit the color
    //  of their parent button (which handles hover/active states
    //  via CSS).
    // ============================================================
    const ICONS = {
      // Main action button icon (floating mic)
      ...
      // Reload button in the header (circular arrow)
      ...
      // Manual search button (magnifier)
      ...
      // Checkmark (used in the toast + copy button confirmation)
      ...
      // "No music" icon for the empty state (mic with slash)
      ...
      // Minus (for the timing pill "−" button)
      ...
      // Plus (for the timing pill "+" button)
      ...
      // Trash (used on the "Reset all settings" button)
      trash: '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>',
    };

    // ============================================================
    //  HTML
    //  The panel markup. Injected into the shadow root by ui.js.
    //  All interactive elements are referenced by id from ui.js
    //  (see the `ui = { ... }` block there for the full list).
    // ============================================================
    const html = `
      <style>${css}</style>
      ...
      <!-- ============================================================
           ...
           Drag to move, click to open/close the panel.
      ...
      <!-- ============================================================
           MAIN PANEL
           Opened from the toggle button, closed via the × icon.
           ============================================================ -->
      <section id="panel" hidden aria-label="Lyrics">

        <!-- HEADER: cover + title + artist on the left, actions +
             mode switcher on the right. The whole header is
             draggable (moves the panel). -->
        <header class="panel-header">
          ...
          <div class="header-right">
            ...
            <!-- Segmented control: Sing Along vs Full Lyrics -->
        ...
        <div id="settings" class="drawer" hidden>
          ...
          <!-- Behavior section: auto-scroll toggle -->
          ...
          <!-- Appearance section: theme dropdown + font size -->
          <div class="settings-section">
        ...
        <!-- STATUS BANNER (info or error). Has a dismiss button. -->
        ...
        <!-- CONTROLS BAR: timing pill + external links + actions -->
        <div id="offset-row" class="controls-bar" hidden>
          ...
          <!-- Streaming links (Apple Music, Spotify). Shown only
               when the song was resolved successfully. -->
        ...
        <!-- SOURCE LINE (data sources, non-interactive) -->
    ...
    // ============================================================
    //  EXPORT
    //  Y.template is consumed by ui.js when building the shadow DOM.
    // ============================================================
